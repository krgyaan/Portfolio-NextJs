import { motion, useScroll } from 'framer-motion'
import { useRef } from 'react'
import LiIcon from './LiIcon'

const Details = ({ pos, com, comLink, time, addr, work }) => {
    const ref = useRef(null);
    return <li ref={ref} className='my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between'>
        <LiIcon refe={ref} />
        <motion.div initial={{y: 50}} whileInView={{y: 0}} transition={{duration: 0.5, type:'spring'}} >
            <h3 className='capitalize font-bold text-2xl'>
                {pos}&nbsp;
                <a href={comLink} target='_blank' className='capitalize text-primary' >
                    @{com}
                </a>
            </h3>
            <span className='capitalize font-medium text-dark/75'>
                {time} | {addr}
            </span>
            <p className='font-medium w-full'>
                {work}
            </p>
        </motion.div>
    </li>
}

const Experience = () => {
    const ref = useRef(null);
    const { scrollYProgress } = useScroll(
        {
            target: ref,
            offset: ["start end", "center start"]
        },
    );
    return (
        <div className='my-64'>
            <h2 className='font-bold text-8xl mb-32 w-full text-center'>
                Experience
            </h2>

            <div className='w-[75%] mx-auto relative' ref={ref} >
                <motion.div className='absolute top-0 left-9 w-[4px] h-full  bg-dark origin-top'
                    style={{ scaleY: scrollYProgress }}
                />
                <ul className='w-full flex items-start justify-between flex-col ml-4'>
                    <Details
                        pos={'Software Engineer'}
                        com={'Google'}
                        comLink={'https://google.com'}
                        time={'2022-Present'}
                        addr={'Mountain View, CA'}
                        work={'Worked on a team responsible for developing new features for Google\'s search engine, including improving the accuracy and relevance of search results and developing new tools for data analysis and visualization.'}
                    />
                    <Details
                        pos={'Intern'}
                        com={'Facebook'}
                        comLink={'https://facebook.com'}
                        time={'Summer-2021'}
                        addr={'Menlo Park, CA'}
                        work={'Worked on a team responsible for developing a new mobile app feature that allowed users to create and share short-form video content, including designing and implementing a new user interface and developing the backend infrastructure to support the feature.'}
                    />
                    <Details
                        pos={'Software Developer'}
                        com={'Amazon'}
                        comLink={'https://amazon.com'}
                        time={'2020-2021'}
                        addr={'Seattle, WA.'}
                        work={"Worked on a team responsible for developing Amazon's mobile app, including  new features such as product recommendations and user reviews, and optimizing the app's performance and reliability."}
                    />
                    <Details
                        pos={'Software Developer Intern'}
                        com={'Microsoft'}
                        comLink={'https://microsoft.com'}
                        time={'Summer-2019'}
                        addr={'Redmond, WA.'}
                        work={"Worked on a team responsible for developing new features for Microsoft's Windows operating system, including implementing a new user interface for a system settings panel and optimizing the performance of a core system component."}
                    />
                    <Details
                        pos={'Teaching Assistant'}
                        com={'MIT'}
                        comLink={'https://mit.com'}
                        time={'Fall-2018'}
                        addr={'Massachusetts Ave, Cambridge, MA.'}
                        work={"Assisted in teaching a course on computer programming, held office hours to help students with assignments, and graded exams and assignments."}
                    />
                </ul>
            </div>
        </div>
    )
}

export default Experience